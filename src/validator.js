const validator =  {
  isValid: function (nuValidar) {
    ...
    for(let index= 0;index< reveNum.length; index++){ //comento esta linea que hace
      //console.log(index%2)
      if (index%2!==0) { 
        card.push(reveNum[index]*2)
        //console.log("impares",reveNum[index]*2)
      }
      else{
        card.push(reveNum[index])
      }
    }
  ...
  maskify: function(numValidar){
    ...
    for (let i = 0; i < numValidar.length -4; i++ ){
